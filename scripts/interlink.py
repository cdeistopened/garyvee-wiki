...
LINKABLE_DIRS = ["episodes", "frameworks", "skills"]

# Directories to process (add links into)
PROCESS_DIRS = ["episodes", "frameworks", "skills"]

def extract_titles(content_dir: Path) -> dict[str, tuple[str, str]]:
    """Build map of display_title -> (slug, folder) for all linkable pages.

    Priority: frameworks > skills > episodes (frameworks/skills links are most useful).
    When the same title exists in multiple folders, frameworks wins.

    For episodes and frameworks: reads *.md files directly.
    For skills: reads skill-name/SKILL.md files.
    Uses 'name' field from frontmatter (or 'title' as fallback).
    """
    titles = {}

    # Process in reverse priority order so higher priority overwrites
    for dirname in reversed(["episodes", "skills", "frameworks"]):
        dirpath = content_dir / dirname
        if not dirpath.exists():
            continue

        # Different file patterns for different directories
        if dirname == "skills":
            # Skills are in subdirectories with SKILL.md
            md_files = list(dirpath.glob("*/SKILL.md"))
        else:
            # Episodes and frameworks are direct *.md files
        ...
        for md_file in md_files:
            ...
            # Use 'name' field (preferred) or fallback to 'title'
            ...
            # Frameworks always win over skills/episodes with same title
            titles[title] = (slug, dirname)

            # For titles with long descriptions like "Title: Description"
            ...
            if ":" in title:
                ...
                if len(short) > 3:
                    ...
                    if short not in titles or dirname == "frameworks":
...
def main():
    ...
    # Show some examples
    episodes = [(t, s) for t, (s, d) in titles.items() if d == "episodes"]
    frameworks = [(t, s) for t, (s, d) in titles.items() if d == "frameworks"]
    skills = [(t, s) for t, (s, d) in titles.items() if d == "skills"]
    print(f"  Episodes: {len(episodes)}, Frameworks: {len(frameworks)}, Skills: {len(skills)}")
    ...
    for dirname in PROCESS_DIRS:
        ...
        # Different file patterns for different directories
        if dirname == "skills":
            files = sorted(dirpath.glob("*/SKILL.md"))
        else:
        ...
        for filepath in files:
            ...
            if count > 0:
                ...
                if args.verbose:
                    if dirname == "skills":
                        print(f"  {filepath.parent.name}/SKILL.md: +{count} links")
                    else:
